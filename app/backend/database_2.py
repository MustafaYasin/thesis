...
def create_table(csv_folder):
    ...
    for file_name in files:
        ...
        for row in csv_data:
            ...
            username=row[0], name=row[1], email=row[2], repo_count=row[3], company=row[4], avatar_url=row[5], hireable=row[6], star_time=row[7])
            conn=engine.connect()
            conn.execute(ins)
            

#Calling function, argument is path of folder where all CSV files are stored
create_table('/Users/myasin/Documents/thesis/app/backend/csv_folder/pytorch_pytorch.csv')




# connection = sqlite3.connect('profiles_dbase.sqlite')
...
#     records = 0
#     for row in file:
#         cursor.executemany('INSERT INTO profiles_dbase.sqlite VALUES (?,?,?,?,?,?,?,?)', row.split(","))
#         connection.commit()
#         records += 1
